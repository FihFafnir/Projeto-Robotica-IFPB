...
class MazeSolver : public LineFollower {
    Stack *path;
    byte sharpLeftTurnSensorPin, sharpRightTurnSensorPin;

public:
    bool isOn;
    MazeSolver(short initialSpeed, byte maxSpeed, byte numberOfSensors,
               int maxPathLength);
...
MazeSolver::MazeSolver(short inicialSpeed, byte maxSpeed, byte numberOfSensors,
                       int maxPathLength)
    : LineFollower(inicialSpeed, maxSpeed, numberOfSensors) {
    path = new Stack(maxPathLength);
}

float MazeSolver::calculateError() {
    readSensors();
    bool sharpLeftTurnSensorValue = !digitalRead(sharpLeftTurnSensorPin),
         sharpRightTurnSensorValue = !digitalRead(sharpRightTurnSensorPin),
         centralSensorValue = sensorsInBlack[2];

    if (sharpLeftTurnSensorValue && centralSensorValue &&
